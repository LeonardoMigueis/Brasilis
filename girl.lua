...
function colisao(dt)
  ...
  for i=#enemy, 1, -1 do 
    ...
    if checaColisao(girl_pos_x, girl_pos_y, imgGirl[1]:getWidth()/16, imgGirl[1]:getHeight()/16, enemies.x-enemyw/16, enemies.y-enemyh/16, enemyw/16,enemyh/16) and Alive then
     table.remove(enemy, i)
     Alive = false
     vidas = vidas - 1
      if vidas < 0 then
